(function() {
	...
	function openTabCallback(anim, idx, tab) {
		if( anim.progress > 60 && !isContentShown ) {
			...
			content.img.style.opacity = content.title.style.opacity = content.subtitle.style.opacity =  content.subtitletwo.style.opacity =  content.deco.style.opacity = content.box.style.opacity = content.number.style.opacity = 0;
			// Show content item.
			...
			anime({
				targets: [content.img, content.title,content.subtitle,content.subtitletwo, content.box, content.number, content.deco],
				easing: 'easeOutExpo',
	...
	function closeTabs() {
		...
		anime({
			targets: [content.deco, content.number, content.box, content.title, content.subtitle,content.subtitletwo,content.img],
